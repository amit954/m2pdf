...
`).trim()}return null}getMessageHtml(){let e=this.#e.getPartByContentType("text","html");if(e&&!e.isAttachment)return e.getBody();let t=this.#e.getPartByContentType("text","plain");return t&&!t.isAttachment?t.getBody().replace(/\r?\n/g,"<br />"):null}}});var u,f=[];function L(o){return o.replace(/[^a-z0-9\s.\-_]/gi,"").replace(/\s+/g," ").trim().replace(/\s/g,"_")||"attachments"}function P(o){if(o===0)return"0 Bytes";let e=Math.floor(Math.log(o)/Math.log(1024));return parseFloat((o/Math.pow(1024,e)).toFixed(2))+" "+["Bytes","KB","MB","GB","TB"][e]}function _(o){return o.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,"")}function v(o,e){let t=document.getElementById("email-content");if(t.innerHTML="",e){let n=document.createElement("div");n.className="email-content-container",n.innerHTML=_(o),t.appendChild(n)}else{let n=document.createElement("pre");n.className="plain-text-content",n.style.whiteSpace="pre-wrap",n.style.margin="0",n.textContent=o,t.appendChild(n)}}async function C(o){if(o&&o.name.toLowerCase().endsWith(".eml"))if(u)try{$("#error-message").hide();let e=await o.arrayBuffer(),t=new u(e);$("#email-subject").text(t.getSubject()||"(No Subject)"),$("#email-from").text(t.getFrom()||"Unknown"),$("#email-to").text(t.getTo()||"Unknown");let n=t.getDate();if(n){let d={weekday:"short",day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0,timeZoneName:"shortOffset"},p=n.toLocaleString("en-US",d).replace(/,/g,"").replace(/(\d{2}):(\d{2}):(\d{2})/,"$1:$2:$3").replace("GMT","");$("#email-date").text(p)}else $("#email-date").text("Unknown");let a=t.getCc();$("#cc-container").toggle(!!a),a&&$("#email-cc").text(a);let i=t.getBcc();$("#bcc-container").toggle(!!i),i&&$("#email-bcc").text(i);let r=t.getReplyTo();$("#reply-to-container").toggle(!!r),r&&$("#email-reply-to").text(r);let s=t.getAttachments();if(f=s,s.length>0){let d=$("#attachments-list");d.empty(),s.forEach(((p,m)=>{let A=P(p.filesize);d.append(`
                    ...
                    `)})),$("#attachments-section").show(),$("#download-all").toggle(s.length>1),s.length>1&&$("#download-all").off("click").on("click",window.downloadAllAttachments)}else $("#attachments-section").hide(),$("#download-all").hide();let c=t.getMessageHtml(),l=t.getMessageText();c?v(c.replace(/(\r\n|\n|\r)/g,"").replace(/\s+/g," ").replace(/> +</g,"><").replace(/<br\s*\/?>/gi,"<br>").replace(/(<br>){3,}/gi,"<br><br>").trim(),!0):v(l?l.replace(/(\r\n|\n|\r)+/g,`
`).replace(/\n\s+\n/g,`

`).replace(/\s+/g," ").replace(/\n{3,}/g,`

`).trim():"(No content)",!1),$("#email-display").show(),setTimeout((()=>{document.getElementById("email-display").scrollIntoView({behavior:"smooth",block:"start"})}),100)}catch(e){$("#error-message").text("Error processing email: "+e.message).show()}else $("#error-message").text("EML parser not loaded. Please check console for errors.").show();else $("#error-message").text("Please drop an .eml file").show()}function U(){let o=$("#email-subject").text(),e=$("#email-from").text(),t=$("#email-to").text(),n=$("#email-date").text(),a=$("#email-cc").text(),i=$("#email-bcc").text(),r=$("#email-reply-to").text(),s=$("#email-content").html(),c="";if($("#attachments-section").is(":visible")){let d=[];$("#attachments-list .attachment-item").each((function(){let p=$(this).find(".attachment-name").text(),m=$(this).find(".attachment-size").text();d.push(`<div style="display:flex;margin:8px 0;"><span style="flex-grow:1;">${p}</span><span style="color:#5f6368;margin-left:10px;">${m}</span></div>`)})),d.length>0&&(c=`
            ...
            `)}let l=window.open("","_blank");l.document.write(`
        <!DOCTYPE html>
        <html>
        <head>
            <title>Print: ${o}</title>
    ...
    `),l.document.close()}function y(){$(".email-header, .email-content").addClass("no-page-break"),$(".email-content").css({"padding-top":"0","margin-top":"0"}),$("#email-display").css({"margin-top":"0","padding-top":"0"}),$("body").addClass("printing"),$("#email-content").height()>1e3&&$("html, body").css("height","auto"),/Chrome/.test(navigator.userAgent)&&($(".email-header").css("position","relative"),$(".email-content").css("position","relative"))}function w(){$("body").removeClass("printing"),$("html, body").css("height",""),$(".email-header").css("position",""),$(".email-content").css("position","")}if($(document).ready((async function(){try{u=(await Promise.resolve().then(()=>(B(),T))).EmlReader}catch(e){$("#error-message").text("Error loading EML parser: "+e.message).show()}$(document).on("drag dragstart dragend dragover dragenter dragleave drop",(function(e){e.preventDefault(),e.stopPropagation()}));let o=$("#drop-zone");o.on("click",(function(){let e=$('<input type="file" accept=".eml" style="display: none">');e.on("change",(function(t){let n=t.target.files[0];n&&C(n)})),e.click()})),o.on("dragover dragenter",(function(){$(this).addClass("drag-over")})).on("dragleave dragend drop",(function(){$(this).removeClass("drag-over")})).on("drop",(function(e){C(e.originalEvent.dataTransfer.files[0])})),$("#print-button").off("click").on("click",(function(){U()}))})),window.matchMedia){let o=window.matchMedia("print");o.addEventListener?o.addEventListener("change",(function(e){e.matches?y():w()})):o.addListener&&o.addListener((function(e){e.matches?y():w()}))}window.onbeforeprint=y,window.onafterprint=w,window.downloadAllAttachments=async function(){let o=new JSZip,e=L($("#email-subject").text()||"attachments");f.forEach((t=>{o.file(t.filename,t.content)}));try{let t=await o.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:9}}),n=URL.createObjectURL(t),a=document.createElement("a");a.href=n,a.download=`${e}_attachments.zip`,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(n)}catch(t){alert("Error creating ZIP file: "+t.message)}},window.downloadAttachment=function(o,e,t){let n=f[t],a=new Blob([n.content],{type:e}),i=URL.createObjectURL(a),r=document.createElement("a");r.href=i,r.download=o,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(i)};})();
/*
